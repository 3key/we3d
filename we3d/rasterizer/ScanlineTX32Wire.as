package we3d.rasterizer 
{
	...
	/**
	* Transparent bitmap textures with software rasterizer and outlines.
	*/
	public class ScanlineTX32Wire extends Rasterizer
	{
		public function ScanlineTX32Wire () {
			splitFace.vLen = 3;
		}
		
		public override function clone () :IRasterizer {
			var r:ScanlineTX32Wire = new ScanlineTX32Wire();
			return r;
		}
					...
					if(y2 > y1) {
						while(i<L) {	
							...
							for(px=li; px<re; px++) {
								...
								if(t == 0xff) {
									bmpref.setPixel(px, i, col);
								}else if(t > 0) {
									col2 = bmpref.getPixel(px, i);
									t2 = 255-t;
									bmpref.setPixel(px, i, 
