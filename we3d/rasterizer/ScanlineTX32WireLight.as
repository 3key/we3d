package we3d.rasterizer 
{
	...
	/**
	* Transparent bitmap textures with software rasterizer, lighting and outlines.
	*/
	public class ScanlineTX32WireLight extends RasterizerLight
	{
		public function ScanlineTX32WireLight () {
			splitFace.vLen = 3;
		}
		
		public override function clone () :IRasterizer {
			var r:ScanlineTX32WireLight = new ScanlineTX32WireLight();
			return r;
		}
					...
					if(y2 > y1) {
						while(i<L) {	
							...
							for(px=li; px<re; px++) {
								...
								if(t == 0xff) {
									bmpref.setPixel(px, i, scaleColor(col, lv, lumi, diffuse));
								}else if(t > 0) {
									col2 = bmpref.getPixel(px, i);
									t2 = 255-t;
									bmpref.setPixel(px, i, scaleColor(
