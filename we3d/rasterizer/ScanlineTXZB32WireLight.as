package we3d.rasterizer 
{
	...
	import we3d.material.BitmapWireLightAttributes;
	...
	/**
	* Transparent bitmap textures with software rasterizer and zbuffer.
	*/
	public class ScanlineTXZB32WireLight extends RasterizerZBufferLight 
	{
		public function ScanlineTXZB32WireLight (zb:ZBuffer=null) {
			splitFace.vLen = 3;
			zbuffer = zb;
		}
		
		public override function clone () :IRasterizer {
			var r:ScanlineTXZB32WireLight = new ScanlineTXZB32WireLight(zbuffer);
			return r;
		}
		...
		{
			var sf:BitmapWireLightAttributes = BitmapWireLightAttributes(material.attributes);
					...
					if(y2 > y1) {
						while(i<L) {	
							...
							for(px=li; px<re; px++) {
								if(zbuf[zbi] <= esz ) {	
									iz = 1/esz;
									col = tex.getPixel32(pu*iz, pv*iz);
									t = col >> 24 & 0xff;
									
									if(t == 0xff) {
										bmpref.setPixel32(px, i, scaleColor32(col, lv, lumi, diffuse));
										zbuf[zbi] = esz;
									}else if(t > 0) {
										col2 = bmpref.getPixel(px, i);
										t2 = 255-t;
										bmpref.setPixel32(px, i, scaleColor32( 0xff << 24 | 
												(0xff0000 & (((col & 0xff0000) * t + (col2 & 0xff0000) * t2) >> 8)) |
												(0x00ff00 & (((col & 0x00ff00) * t + (col2 & 0x00ff00) * t2) >> 8)) |
												(0x0000ff & (((col & 0x0000ff) * t + (col2 & 0x0000ff) * t2) >> 8)) , lv, lumi, diffuse));
									}
